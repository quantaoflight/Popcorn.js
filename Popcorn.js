 ...
 * 
 * Mihir Deshpande - June 2012
 * Flickr search Improved, HTML5 Video tagging  plugin using Canvas
 * implemented
 */
...
//code added by Mihir
	...
	//http://api.flickr.com/services/rest/?api_key=ccabe24d5848521fbef7adeb2070ecb0&text=badami&sort=interestingness-desc&method=flickr.photos.search
        //http://api.flickr.com/services/rest/?&api_key=ccabe24d5848521fbef7adeb2070ecb0&text=badami&sort=interestingness-desc&method=flickr.photos.search&format=json&jsoncallback=flickr
//popcorn.js (line 3560)

	_uri="http://api.flickr.com/services/rest/?";
        _uri += "&api_key=" + options.apikey +"&text=" + options.searchtag + "&sort=interestingness-desc&method=flickr.photos.search" + 	"&format=json&jsoncallback=flickr";
        console.log(_uri);
      ...
      Popcorn.getJSONP( _uri, function( data ) {
        ...
        var fragment = document.createElement( "div" );
         fragment.setAttribute("id" , "searchres");
        fragment.innerHTML = "<p style='padding:" + _padding + ";'>" + data.title + "<p/>";
         console.log(data.photos.photo);
         Popcorn.forEach(data.photos.photo,function(item , i)
       	 {
			
		if(i < data.photos.photo.length)
		{
			getphoto(item.id,item.secret,fragment);
		} 		
     ...
     if(options.searchtag && options.apikey)
     {
        flickrsearch();
     }
     var getphoto = function(id,secret,fragment){
	
	_uri ="http://api.flickr.com/services/rest/?api_key="+options.apikey + "&format=json&photo_id=" +id + 		   		       "&method=flickr.photos.getSizes&jsoncallback=flickr";
		Popcorn.getJSONP( _uri, function( data ) {
		//console.log(data);
		
		//console.log(data.sizes.size[3].source);	
		var item = data.sizes.size[3];	
                _link = document.createElement( "a" );
            	_link.setAttribute( "href", item.url );
            _link.setAttribute( "target", "_blank" );
            _image = document.createElement( "img" );
            _image.setAttribute( "src", item.source );
            _image.setAttribute( "height",item.height );
            _image.setAttribute( "width", item.width);
            _image.setAttribute( "style", "border:" + _border + ";padding:" + _padding );
            _link.appendChild( _image );
            fragment.appendChild( _link );  
		
/*
		Popcorn.forEach( data.sizes.size, function ( item, i ) {
			console.log(item._content);

            _link = document.createElement( "a" );
            _link.setAttribute( "href", item._content );
            _link.setAttribute( "target", "_blank" );
            _image = document.createElement( "img" );
            /*_image.setAttribute( "src", item._content );
